    ...
    <div class="container">
      <div class="header">
        ...
        <div>
          ...
          <span class="button" @click="addEmployee" v-if="userRole == 1">添加成员</span>
        </div>
      </div>
      <el-form :inline="true" :model="searchForm" style="padding-top: 20px;">
        <el-form-item label="部门：">
          <el-select v-model="searchForm.departemnt" filterable clearable placeholder="请选择">
            <el-option
        ...
        <el-form-item style="float: right;">
          <el-button type="primary" @click="getContactList">搜索</el-button>
        </el-form-item>
      </el-form>
      <el-table
        :data="tableData"
      ...
      <el-dialog title="成员详情" width="800px" @close="editorMethods(true)" :visible.sync="dialogDetailVisible">
        ...
        <div class="dialog-content">
          <img src="~assets/img/common/default_avatar.png" alt="头像">
          <div class="info">
            <ul v-if="editor">
              <li>{{staffName}}</li>
      ...
      <el-dialog
        ...
        width="500px"
        center>
        <el-form ref="phoneForm" :model="phoneForm" :rules="rules" style="margin-top: 20px;" label-width="200px">
          ...
          <el-form-item label="选择部门" prop="department">
            <el-select v-model="phoneForm.department" filterable placeholder="请选择部门">
              <el-option
                v-for="item in options"
                :key="item.organizationId"
                :label="item.organizationName"
...
export default {
  ...
  methods: {
    async getContactList () {
      await apiDepartment.getContactList({
        'type': 'list',
        'organizationId': this.searchForm.departemnt,
    ...
    handleClick (row) {
      this.organizationNameDetail = row.organizationName
    ...
    submitSecond (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.employeeForm.organizationId = this.phoneForm.department
          this.userAdd()
        }
      })
    },
    ...
    async hanldeSave () {
      let organizationId = Number(this.organizationNameDetail) ? this.organizationNameDetail : this.phoneForm.department
